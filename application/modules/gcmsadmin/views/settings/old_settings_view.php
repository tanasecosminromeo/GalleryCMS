<?php $this->load->view('header'); ?>
	...
	<div id="settings-update" class="form-page">
		<form action="<?php echo site_url("settings/update")?>" method="post" name="update_settings">
			...
			<div>
				<span>Crop Thumbnails?<br /></span>
				<?php $crp = $rows[0]->crop; ?>
				<label> Yes <input name="crop" type="radio" id="crop" value="Y" <?php if ( $crp == "Y" ) echo 'checked="checked"' ?> /></label>
  				<label> No <input name="crop" type="radio" id="crop" value="N" <?php if ( $crp == "N" ) echo 'checked="checked"' ?> /></label>
			</div>
			<div>
